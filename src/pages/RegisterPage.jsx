...
export default function RegisterPage() {
  ...
  async function handleRegister(e) {
    ...
    setBusy(true);
    // Pass all profile fields as metadata — provision_new_user() trigger reads them
    ...
    // Do NOT manually insert into the user table here.
    // The provision_new_user() PostgreSQL trigger fires automatically on
    // auth.users INSERT and creates the USER/INACTIVE row with VIEW-only rights.
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#f5f4f0", fontFamily: "'DM Sans','Segoe UI',sans-serif", padding: "20px" }}>
      <style>{`
        ...
        .auth-card { width: 100%; max-width: 420px; background: #fff; border-radius: 16px; padding: 40px 36px; border: 1px solid #e0ddd8; box-shadow: 0 2px 24px rgba(0,0,0,.07); box-sizing: border-box; }
        ...
        .form-group label { font-size: 12px; color: #666; display: block; margin-bottom: 4px; font-weight: 400; }
        ...
        .review-text { font-size: 12px; color: #999; text-align: center; margin: 20px 0 0; line-height: 1.4; }
      `}</style>
      ...
      <div className="auth-card">
        ...
        {success ? (
          <div style={{ textAlign: "center" }}>
            ...
            <button className="btn-main" onClick={() => navigate("/login")}>Back to Sign In</button>
          </div>
        ) : (
          <>
            <div className="tab-group">
              <button className="tab-btn" onClick={() => navigate("/login")} style={{ background: "transparent", color: "#888" }}>
                Sign In
              </button>
            ...
            <form onSubmit={handleRegister}>
              <div className="form-row">
                <div className="form-group">
                  ...
                  <input className="auth-input" type="text" value={firstName} onChange={e => setFirstName(e.target.value)} placeholder="e.g. Juan" required />
                </div>
                <div className="form-group">
                  <label>Last Name</label>
                  <input className="auth-input" type="text" value={lastName} onChange={e => setLastName(e.target.value)} placeholder="e.g. Dela Cruz" required />
                </div>
              </div>
